...
import {
  ChevronLeft, ChevronRight, MapPin, Pencil, PlusCircle, PawPrint,
  Loader2, X, Plus, Clock, CheckCircle2, AlertCircle, CalendarCheck,
  ShieldAlert,
} from "lucide-react";
...
const EMPTY_FORM: CreatePetRequest = {
  ...
  birth_date: "", notes: "", photo_url: "", weight_kg: undefined,
};
...
function AddPetModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const inputCls = "w-full rounded-lg border border-border bg-muted/40 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary";
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-background p-6 shadow-2xl ring-1 ring-border">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input className={inputCls} value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ej: Luna" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-semibold">Especie</label>
            <select className={inputCls} value={form.species}
              onChange={(e) => setForm({ ...form, species: e.target.value as PetSpecies, breed: "" })}>
          ...
          <div>
            ...
            <select className={inputCls} value={form.sex ?? "male"}
          ...
          <div className="col-span-2">
            ...
            {breeds.length > 0 ? (
              <select className={inputCls} value={form.breed ?? ""}
              ...
              <input className={inputCls} value={form.breed ?? ""}
                onChange={(e) => setForm({ ...form, breed: e.target.value })}
                placeholder={breedsLoading ? "Cargando razas…" : "Ej: Labrador"} disabled={breedsLoading} />
            )}
          </div>
          <div>
            <label className="mb-1 block text-sm font-semibold">Peso (kg)</label>
            <input type="number" min="0" step="0.1" className={inputCls}
              value={form.weight_kg ?? ""} placeholder="Ej: 5.2"
        ...
        <div className="mt-5 flex gap-3">
          ...
          <Button className="flex-1" onClick={handleSubmit} disabled={submitting}>
            {submitting ? <Loader2 className="size-4 animate-spin" /> : "Registrar mascota"}
          </Button>
        </div>
      </div>
    </div>
...
}: { selectedAddress: AddressOut | null; onSelect: (a: AddressOut) => void }) {
  ...
  if (addresses.length === 0) return (
    <>
      ...
      <AddressFormDialog open={formOpen} onOpenChange={setFormOpen}
        onSubmit={async (p: AddressCreateIn) => { await create(p); setFormOpen(false); }} />
    </>
  );
      ...
      <button onClick={() => setPickerOpen((p) => !p)}
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 text-left">
            <p className="truncate text-sm font-medium">{selectedAddress?.address_line ?? "Selecciona una dirección"}</p>
            {selectedAddress?.reference && (
              <p className="truncate text-xs text-muted-foreground">{selectedAddress.reference}</p>
            )}
          </div>
        </div>
        ...
        <div className="mt-2 overflow-hidden rounded-xl border bg-background shadow-lg">
          {addresses.map((addr) => (
            <button key={addr.id}
              ...
              className={cn("flex w-full items-start gap-3 px-4 py-3 text-left text-sm transition-colors hover:bg-muted",
              ...
              <div className="min-w-0">
                ...
                {addr.is_default && <span className="text-xs font-semibold text-primary">Predeterminada</span>}
              </div>
            </button>
          ))}
          <button onClick={() => { setPickerOpen(false); setFormOpen(true); }}
...
function HoldView({ hold, secondsLeft, loading, error, onConfirm, onCancel }: HoldViewProps) {
    ...
    <div className="flex flex-col gap-4">
        ...
        <div>
          <p className="font-extrabold text-foreground">
            {isConfirmed ? "¡Reserva confirmada!" :
              expired ? "Reserva temporal expirada" : "Reserva temporal activa"}
          </p>
      ...
      {hold.status === "held" && (
        <div className={cn(
          ...
          <div className="flex items-center gap-2">
            <Clock className={cn("size-4", urgency ? "text-orange-500" : "text-muted-foreground")} />
          ...
          <span className={cn(
            ...
            expired ? "text-destructive" : urgency ? "text-orange-500" : "text-foreground"
          )}>
      ...
      {!isConfirmed && (
        <div className="flex flex-col gap-2">
          ...
          <Button variant="outline" className="w-full rounded-xl" onClick={onCancel} disabled={loading}>
      ...
      {isConfirmed && (
        <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-center text-sm font-medium text-green-700">
          ✅ Tu reserva ha sido confirmada. El equipo de Paku se pondrá en contacto contigo.
        </div>
      )}
    </div>
  );
...
function BookingCalendar({ selectedDate, onSelectDate, serviceId }: BookingCalendarProps) {
  const { slotsLoading, slotsError, fetchAvailability, getSlotForDate } = useBooking();
  ...
  return (
    <div className="rounded-2xl bg-muted/40 p-6 ring-1 ring-border sm:p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h3 className="text-xl font-extrabold capitalize sm:text-2xl">{monthLabel}</h3>
      ...
      <div className="rounded-xl bg-card p-4 sm:p-6">
        ...
        <div className="grid grid-cols-7 gap-y-1">
          {gridCells.map(({ day, iso, isCurrentMonth }, i) => {
            ...
            const isUnavailable = slot ? (slot.available <= 0 || !slot.is_active) : false;
              ...
              <div key={i} className="relative flex flex-col items-center">
                <button
                  ...
                  className={cn(
                    ...
                    isUnavailable && isCurrentMonth && "text-muted-foreground/50 line-through cursor-not-allowed",
      ...
      {(() => {
        ...
        const isSelectedInCurrentMonth = sy === viewYear && sm === viewMonth + 1;
        ...
        return (
          <div className="mt-4 flex items-center justify-between rounded-xl bg-primary/5 px-4 py-3">
            ...
            <span className={cn(
              "text-xs font-bold",
              slot.available === 0 ? "text-destructive" :
                slot.available <= 2 ? "text-orange-500" : "text-green-600"
            )}>
              {slot.available === 0 ? "Sin cupos" : `${slot.available} cupo${slot.available !== 1 ? "s" : ""}`}
            </span>
          </div>
        );
      })()}
...
function BookingSidebar({ selectedDate, onDateChange }: BookingSidebarProps) {
  ...
  const displayName = user?.first_name ?? user?.email?.split("@")[0] ?? "Usuario";

  // Perfil incompleto
  const profileIncomplete = isAuthenticated && user &&
    (!user.phone || !user.sex || !user.birth_date);

  const handleSelectAddress = useCallback((a: AddressOut) => setSelectedAddress(a), []);
    ...
    try {
      ...
      if (err instanceof ApiCallError) {
        if (err.status === 409 || err.status === 400) {
          setBookingError("Esta fecha ya no tiene disponibilidad. Por favor, elige otra.");
  ...
  async function handleConfirm() {
    try {
      ...
      if (err instanceof ApiCallError && err.status === 409) {
        setBookingError("El cupo fue tomado por otro usuario. Por favor, elige otra fecha.");
        onDateChange(null);
      }
    }
  }
  ...
  const canBook = isAuthenticated && !profileIncomplete && selectedPetId && selectedDate && !activeHold;
      ...
      <div className="rounded-2xl bg-muted/40 p-6 ring-1 ring-border sm:p-8">
        ...
        {profileIncomplete && (
          <a href="/account/profile"
            ...
            <div>
              ...
              <p className="text-xs">Necesitamos tu teléfono, sexo y fecha de nacimiento.</p>
            </div>
          </a>
        )}
        ...
        {!activeHold && (
          <div className="mb-6">
            ...
            <AddressSelector selectedAddress={selectedAddress} onSelect={handleSelectAddress} />
          </div>
        )}
          ...
          <>
            ...
            <div>
              ...
              {petsLoading && (
                <div className="flex items-center gap-2 rounded-xl bg-muted/60 px-4 py-6 text-sm text-muted-foreground">
                  <Loader2 className="size-4 animate-spin" /> Cargando mascotas...
                </div>
              )}
              {!petsLoading && petsError && (
                <p className="rounded-xl bg-destructive/10 px-4 py-3 text-sm text-destructive">{petsError}</p>
              )}
              ...
              {!petsLoading && !petsError && isAuthenticated && pets.length === 0 && (
                <div className="rounded-xl border-2 border-dashed border-border bg-card px-4 py-6 text-center text-sm text-muted-foreground">
                    ...
                    return (
                      <button key={pet.id} onClick={() => setSelectedPetId(pet.id)}
                        ...
                        <div className={cn(
                          ...
                          {photoUrl ? (
                            <Image src={photoUrl} alt={pet.name} fill className="object-cover" sizes="64px" />
                          ) : (
                        ...
                        <h4 className="text-sm font-bold leading-tight">{pet.name}</h4>
                        <p className="text-xs text-muted-foreground">
                          {speciesLabel(pet.species)}{pet.breed ? ` · ${pet.breed}` : ""}
                        </p>
                        <p className="mt-0.5 text-xs text-muted-foreground/70">{calcPetAge(pet.birth_date)}</p>
                      </button>
                    );
                  })}
            ...
            {selectedDate && selectedPetId && (
              <div className="mt-4 rounded-xl border border-border/60 bg-card px-4 py-3">
                <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">Resumen</p>
                <p className="text-sm font-semibold text-foreground capitalize">{formatDateLong(selectedDate)}</p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Mascota: {pets.find((p) => p.id === selectedPetId)?.name ?? "—"}
                </p>
              </div>
            )}
          </>
