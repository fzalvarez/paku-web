  ...
  return (
    <div className="flex items-center gap-4 rounded-xl bg-muted/50 p-4 transition-colors hover:bg-muted">
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-muted-foreground">{calcPetAge(pet.birth_date)}</p>
      </div>
    </div>
  );
}
  ...
  if (!isAuthenticated) {
    return (
      ...
      >
        ...
        <span className="text-xl font-bold tracking-tight">Registrar nueva mascota</span>
        <span className="mt-2 text-sm text-muted-foreground">Inicia sesión para ver tus mascotas</span>
      </button>
    );
  }
  ...
  if (loading) {
    return (
      <div className="flex min-h-80 w-full flex-col items-center justify-center rounded-2xl border border-border bg-muted/40 p-8">
        ...
        <p className="mt-4 text-sm text-muted-foreground">Cargando mascotas...</p>
      </div>
    );
  }
  ...
  if (pets.length === 0) {
    return (
      ...
      >
        ...
        <span className="text-xl font-bold tracking-tight">Registrar nueva mascota</span>
        <span className="mt-2 text-sm text-muted-foreground">Aún no tienes mascotas registradas</span>
      </button>
    );
  }
  ...
  return (
    <div className="flex min-h-80 w-full flex-col rounded-2xl border border-border bg-muted/40 p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary">{pets.length}</span>
      </div>
  ...
  return (
    <div className="flex flex-col justify-between rounded-2xl bg-card p-8 shadow-sm ring-1 ring-border">
      <div>
        ...
        <h3 className="mb-4 text-2xl font-bold">Personalizacion de Servicios</h3>
        <p className="leading-relaxed text-muted-foreground">
          Cada mascota es unica. Personalizamos el grooming basandonos en la raza, tipo de pelaje y necesidades especificas de salud de tu companero.
        </p>
      </div>
    ...
    <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      ...
      <header className="mb-12">
        ...
        <p className="max-w-2xl text-lg text-muted-foreground">
          Gestiona los perfiles de tus mascotas y descubre servicios diseñados específicamente para tus compañeros peludos.
        </p>
      </header>
