      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-background p-6 shadow-2xl ring-1 ring-border">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input className={inputCls} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ej: Luna" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-semibold">Especie</label>
            <select className={inputCls} value={form.species} onChange={(e) => setForm({ ...form, species: e.target.value as PetSpecies, breed: "" })}>
          ...
          <div>
            ...
            <select className={inputCls} value={form.sex ?? "male"} onChange={(e) => setForm({ ...form, sex: e.target.value as PetSex })}>
          ...
          <div className="col-span-2">
            ...
            {breeds.length > 0 ? (
              <select className={inputCls} value={form.breed ?? ""} onChange={(e) => setForm({ ...form, breed: e.target.value })}>
                <option value="">Sin especificar</option>
                {breeds.map((b) => <option key={b.id} value={b.name}>{b.name}</option>)}
              </select>
            ) : (
              <input className={inputCls} value={form.breed ?? ""} onChange={(e) => setForm({ ...form, breed: e.target.value })} placeholder="Ej: Labrador" disabled={breedsLoading} />
            )}
          </div>
          <div>
            <label className="mb-1 block text-sm font-semibold">Peso (kg)</label>
            <input type="number" min="0" step="0.1" className={inputCls} value={form.weight_kg ?? ""} placeholder="Ej: 5.2"
          ...
          <div>
            ...
            <input type="date" className={inputCls} value={form.birth_date ?? ""} onChange={(e) => setForm({ ...form, birth_date: e.target.value })} />
          </div>
        </div>
        {error && <p className="mt-3 text-sm text-destructive">{error}</p>}
        <div className="mt-5 flex gap-3">
          <button onClick={onClose} disabled={submitting} className="flex-1 rounded-xl border border-border px-4 py-2.5 text-sm font-semibold hover:bg-muted">Cancelar</button>
          <button onClick={handleSubmit} disabled={submitting} className="flex-1 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground disabled:opacity-50">
            {submitting ? <Loader2 className="mx-auto size-4 animate-spin" /> : "Registrar mascota"}
          </button>
        </div>
      </div>
    </div>
...
export function StepSelectPet({ selectedPetId, onSelectPet, onNext }: StepSelectPetProps) {
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-extrabold">¿Para qué mascota es el servicio?</h2>
        <p className="mt-1 text-sm text-muted-foreground">Selecciona la mascota que recibirá el servicio.</p>
      </div>
      ...
      {!loading && !error && (
        <div className="flex flex-wrap gap-4">
            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  {photoUrl ? (
                    <Image src={photoUrl} alt={pet.name} fill className="object-cover" sizes="80px" />
                  ) : (
          ...
          >
            ...
            <span className="text-sm font-semibold text-muted-foreground">Nueva mascota</span>
          </button>
        </div>
      )}
